...
RSpec.describe Game do
  ...
  it '4. bot only has columns A through G as options to select' do
    ...
    expect(game.column_choices).to eq(%w[A B C D E F G])
  end
  ...
  it '5. has random bot selection by shuffling the choices A through G and always returning first element' do
    ...
    expect(game.column_choices).to eq(%w[A B C D E F G])
    game.bot_selection
    expect(game.column_choices).not_to eq(%w[A B C D E F G]) # may possibly randomly stay the same but statistically unlikely. If it ever breaks just rerun it.
    expect(game.column_choices).to include('A', 'B', 'C', 'D', 'E', 'F', 'G')
    bot_selection_1 = game.column_choices[0]
    game.bot_selection
    expect(game.column_choices).not_to eq(%w[A B C D E F G])
    expect(game.column_choices).to include('A', 'B', 'C', 'D', 'E', 'F', 'G')
